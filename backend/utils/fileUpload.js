...
async function uploadFile(partData, options = {}) {
  ...
  if (!partData || !partData.buffer || !partData.filename) {
    throw new Error('Invalid file data provided for upload. Missing buffer or filename.');
  }
  ...
  if (fileSizeInBytes > maxSizeInBytes) {
    throw new Error(`File size (${(fileSizeInBytes / (1024 * 1024)).toFixed(2)} MB) exceeds the limit of ${maxSizeMB} MB.`);
  }
